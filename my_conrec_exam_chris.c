...
extern void print_matrix(char *desc, lapack_int m, lapack_int n,
                             lapack_complex_double * a, lapack_int lda);
extern void print_rmatrix(char *desc, lapack_int m, lapack_int n, double *a,
                              lapack_int lda);
void prtdat(int nx, int ny, double *u1, char *fnam, double *, double *);
...
int main()
{
    ...
    double x_min, x_max, y_min, y_max, stepx,	/* step size for finding gridpoints coordinates in x and y dimension. */
    stepy;
    double e = 0.1;
    ...
    for (i = 0; i < SCALE * NGRID; i++) {
        if ((data[i] = malloc(SCALE * NGRID * sizeof(double))) == NULL) {
            fprintf(stderr,
                    "Failed to malloc space for the data\n");
            exit(-1);
        }
    }
    for (i = 0; i < SCALE * NGRID; i++) {
    ...
    */
    ...
    for (iy = 0; iy < NGRID * NGRID; iy++) {
        ...
        printf
        ("LAPACKE_zgesvd (row-major, high-level) Example Program Results(%d,%d)\n",
         iy / NGRID, iy % NGRID);
        /* Compute SVD */
        info =
            LAPACKE_zgesvd(LAPACK_ROW_MAJOR, 'N', 'N', m, n, temp, lda,
                           s, NULL, ldu, NULL, ldvt, superb);
        //~
...
//                                      printf(" s[ %d ] = %f\n", i, s[ i ] );
        ...
        if (s[m - 1] <= e) {
            printf
            ("THIS ELEMENT BELONGS TO PSEUDOSPECTRA (%d,%d):%6.10f\n",
     ...
     */
    double zmin, zmax;
    prtdat(NGRID, NGRID, plot, "svd_no_disks.data", &zmin, &zmax);
    printf("Max: %f \n", zmax);
    printf("Min: %f \n", zmin);
    ...
    /* Parse again
    prtdat(NGRID*SCALE, NGRID*SCALE, data, "svd_no_disks_scaled.data", &zmin, &zmax);
    printf("Max: %f \n", zmax);
    printf("Min: %f \n", zmin); */
     ...
     */
    const int F_RAD = 0; // <- 0 for no smoothening
    const int NX = NGRID;
    const int NY = NGRID;
    double sum;
    int nn;
    ...
    for (i = 0; i < SCALE * NX; i++) {
        for (j = 0; j < NY * SCALE; j++) {
            nn = 0;
            sum = 0;
            for (ii = -F_RAD; ii <= F_RAD; ii++) {
                for (jj = -F_RAD; jj <= F_RAD; jj++) {
            ...
            if (nn <= 0) {
                fprintf(stderr,
                        "No cells averaged, this shouldn't happen!\n");
                exit(-1);
            }
            data[i][j] = sum / nn;
        }
    ...
    contours[0] = zmax/50000.;
    contours[1] = zmax/3000.;
    contours[2] = zmax/80.;
    contours[3] = zmax/5.;
    contours[4] = zmax;
...
void print_matrix(char *desc, lapack_int m, lapack_int n,
                  lapack_complex_double * a, lapack_int lda)
{
    lapack_int i, j;
    printf("\n %s\n", desc);
...
void prtdat(int nx, int ny, double *u1, char *fnam, double *min, double *max)
{
    int ix, iy;
    *min = u1[0];
    *max = u1[0];
    ...
    for (ix = 0; ix < nx * ny; ix++) {
        *max = u1[ix] > *max ? u1[ix] : *max;
        *min = u1[ix] < *max ? u1[ix] : *min;
...
void drawline(double x1, double y1, double x2, double y2, double z)
{
    ...
    Draw_Line(image, SCALE * NGRID, SCALE * NGRID, (int)x1, (int)y1,
              (int)x2, (int)y2, black);
    vectorsdrawn++;
}
